...
public class BookController {
    ...
    public String add(@RequestParam String name, @RequestParam String author,
                      @RequestParam("author_id") Integer authorId, Map<String, Object> model) {
        Book book = new Book();
        book.setName(name);
        book.setAuthor(author);
        Author newAuthor = new Author();
        newAuthor.setAuthorId(authorId);
        book.setAuthorId(newAuthor);// Присваиваем объекту Book соответствующего автора по его ID
        bookRepos.createBook(book);
        Iterable<Book> books = bookRepos.getAllBooks();
        model.put("AllBooks", books);
        return "main";
